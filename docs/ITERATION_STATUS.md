...
- Stopped widening the direct DeepSeek-to-`StoryboardPlan` parser as the main
  ...
  provider-facing `StoryboardPlanDraft` and compiles it deterministically into
  the existing strict `StoryboardPlan` boundary.
- Added `src/lib/storyboard-plan-draft-schema.ts` as the LLM-facing draft
  contract. Draft segments carry semantic fields only: title/purpose,
  narration text, visual brief, simple `visualKind`, and optional steps or
  terminal commands.
- Added `src/lib/storyboard-plan-draft-compiler.ts` as a pure deterministic
  compiler from Draft to `StoryboardPlan`. It owns ids, contiguous order,
  template choice, strategy decisions, bounded procedural generator payloads,
  generator refs, and beat timing.
- Added `src/lib/deepseek/parse-storyboard-plan-draft.ts` and changed the
  DeepSeek storyboard facade to parse Draft first, then fall back to the
  existing strict `StoryboardPlan` parser for compatibility with older or
  already-running prompts.
- Narrowed the DeepSeek full-project and selected-segment planner prompts so
  the model no longer has to emit `templateId`, `strategyDecision`,
  `proceduralGenerator`, refs, beats, or `atFrame`. Those are now generated by
  code for the bounded workflow, line-path, and terminal cases.
- Added `npm run smoke:storyboard-draft` to cover the real system-flow prompt
  shape as a Draft: node graph, line path, terminal session, and overlong
  workflow steps remain bounded by deterministic compilation.

Validation performed so far:
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:storyboard-draft'` (red first for missing draft modules, then green)
...
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:staged-fixtures'` (red first for old prompt-surface guards, then green)
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npx tsc --noEmit'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run lint'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run build'`

## Previous continuation — DeepSeek procedural generator bounded recovery hardening

- Fixed another observed DeepSeek planner near-miss for the system-flow prompt
  path where a `scene-graph` + `procedural_generator` storyboard plan failed
  strict schema validation before staged generation could continue.
- Hardened the same parser boundary for the latest multi-segment system-flow
  failure where every procedural generator segment omitted `title` and a
  terminal/line-style generator emitted more beats than the current schema
  permits.
- Kept the fix at the provider parser boundary only. The parser now recovers
  these bounded JSON-mode shapes before running the full `StoryboardPlan`
  schema:
  - object-shaped top-level `globalStyle` is converted to a compact
    semicolon-separated string using simple scalar fields
  - missing `proceduralGenerator.title` still defaults from the segment title
    or fallback segment text
  - generator beat timing aliases `time`, `startFrame`, or `frame` normalize
    to `atFrame`
  - generator beat-level `duration` is dropped because the current bounded
    generator beat schemas do not model per-beat duration
  - generator beats are trimmed only for known generator ids at their current
    schema limits: `node-graph-flow` 20, `line-path-flow` 16, and
    `terminal-session` 16
- If bounded recovery still fails, `StoryboardPlanParseError` now reports the
  post-recovery validation issues instead of showing stale pre-recovery errors
  such as missing `title` after that field has already been filled.
- Existing strict failures remain unchanged for unsupported generator ids,
  bad refs, unsupported strategies, arbitrary generator fields outside the
  known beat aliases, and free-form provider output.
- Added storyboard parser smoke coverage for the reported system-flow prompt
  shapes: object `globalStyle`, missing generator title, beat `startFrame`,
  extra beat `duration`, three procedural-generator segments, and an overlong
  terminal-session beat list.

Validation performed:
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:storyboard-parser'` (red first for the reported validation shape, then green)
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:provider-boundary'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:staged-fixtures'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npx tsc --noEmit'`

## Previous continuation — Visual review repair action distinction fix

- Fixed a Studio UX ambiguity where `套用修复指令` and `立即修复分镜` both appeared
  to do the same thing because the immediate repair path reused
  `applyVisualReviewRepairPrompt()`, which selects the segment, fills the
  revision prompt, and scrolls/focuses the segment editor.
- Kept `套用修复指令` as the explicit prompt-prep action: it still selects the
  target segment, fills the structured repair prompt into the segment editor,
  and focuses the revision textarea.
- Changed `立即修复分镜` so it no longer calls the prompt-prep/focus helper.
  It selects the target segment and immediately runs deterministic repair or
  the existing fallback selected-segment regeneration path, with the result
  reported in the visual-review panel.
- Updated visual-review UI smoke coverage to assert the two actions remain
  distinct: prompt-prep may jump to the segment editor, while immediate repair
  must not call `applyVisualReviewRepairPrompt()`.
- Updated the panel helper copy so the user-facing difference is explicit:
  applying a repair instruction only fills the editor, while immediate repair
  executes now and reports the result in the panel.

Validation performed:
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:visual-review-ui'` (red first for missing distinct action copy and immediate repair still using the prompt-prep helper, then green)
...
## Previous continuation — Visual review still-analysis diagnostics merge hardening

- Continued the current Phase 6 manual review/repair closure by making the
  still-analysis findings merge path directly testable outside the
  `/api/visual-review/stills` route handler.
- Added `mergeVisualReviewStillAnalysisDiagnostics()` to the staged
  visual-review diagnostics boundary. It combines static preflight findings
  with still-analysis findings, preserves the existing representative
  `reviewFrames[]`, and recomputes warning/error/finding counts from the
  merged result.
- Split still-analysis finding construction into a pure
  `src/lib/visual-review-still-findings.ts` helper so the route can keep
  returning source-attributed findings without coupling that behavior to the
  Node-only PNG analyzer module.
- Added `npm run smoke:visual-review-diagnostics` to cover the behavior:
  an abnormal still creates one warning with `stillId`, `reviewReason`, frame,
  and target segment attribution, while an `analyzed` still does not create a
  review finding.
- Kept the product boundary unchanged: this does not add browser/canvas
  review, automatic repair loops, or silent segment rewriting.

Validation performed:
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:visual-review-diagnostics'` (red first for missing merge helper, then green)
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:visual-repair'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run smoke:visual-review-ui'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npx tsc --noEmit'`
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npm run lint'`
...
- `docker compose run --rm web bash -lc '[ -d /workspace/node_modules/next ] || npm install; npx prettier --check package.json scripts/visual-review-diagnostics-smoke.mjs scripts/visual-review-ui-source-smoke.mjs src/lib/staged-generation/visual-review.ts src/lib/visual-review-still-findings.ts src/lib/visual-review-still-analysis.ts src/app/api/visual-review/stills/route.ts docs/ITERATION_STATUS.md'`
- `git diff --check`
...
- Continued Phase 6 inside the existing manual still-extraction review action
  by extending server-side PNG still analysis from near-blank, low-contrast,
  unsafe-margin, and fine-detail detection to bounded letterbox/pillarbox
  empty-border detection.
- Added strict per-still `borderBandRatio` metadata and a `letterbox_frame`
  analysis status while preserving the existing `blankFrameScore`,
  `contrastScore`, `edgeContentRatio`, `fineDetailRatio`, dominant-color,
  luma-range, pixel-count, and unsupported states.
- Added letterbox/pillarbox still findings by merging still-analysis warnings
  back into the returned `visualReview.findings`; this remains a manual
  review result and does not trigger automatic segment repair.
- Updated the Studio visual-review panel to show each still's band score next
...
## Latest continuation — Visual Review fine-detail still analysis v1

- Continued Phase 6 inside the existing manual still-extraction review action
  by extending server-side PNG still analysis from near-blank, low-contrast,
  and unsafe-margin detection to bounded fine-detail density detection.
- Added strict per-still `fineDetailRatio` metadata and a
  `fine_detail_frame` analysis status while preserving the existing
  `blankFrameScore`, `contrastScore`, `edgeContentRatio`, dominant-color,
  luma-range, pixel-count, and unsupported states.
- Added fine-detail still findings by merging still-analysis warnings back
  into the returned `visualReview.findings`; this remains a manual review
  result and does not trigger automatic segment repair.
- Updated the Studio visual-review panel to show each still's detail score
...
- Continued Phase 6 inside the existing manual still-extraction review action
  ...
  bounded low-contrast detection.
- Added strict per-still `contrastScore` metadata and a
  `low_contrast_frame` analysis status while preserving the existing
  `blankFrameScore`, dominant-color, luma-range, pixel-count, and unsupported
  states.
- Added low-contrast still findings by merging still-analysis warnings back
  into the returned `visualReview.findings`; this remains a manual review
  result and does not trigger automatic segment repair.
- Updated the Studio visual-review panel to show each still's contrast score
  next to the generated thumbnail.
- Browser/canvas visual analysis and automatic target-segment repair remain
  deferred.
...
## Previous continuation — Visual Review still analysis v1

- Continued Phase 6 inside the explicit still-extraction review action by
  adding server-side PNG still analysis after each `renderStill` call.
- Added strict per-still `analysis` metadata with `blankFrameScore`,
  `dominantColorRatio`, `lumaRange`, `pixelCount`, and analysis status.
- Added bounded near-blank frame findings by merging still-analysis warnings
  back into the returned `visualReview.findings`; this remains a manual review
  result and does not trigger automatic segment repair.
- Updated the Studio visual-review panel to show each still's analysis status
  and blank-frame score next to the generated thumbnail.
- Browser/canvas visual analysis and automatic target-segment repair remain
  deferred.
