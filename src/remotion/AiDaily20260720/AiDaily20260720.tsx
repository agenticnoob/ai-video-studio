...
const ProfileCaption: FC<{ scene: AiDaily20260720Scene }> = ({ scene }) => {
...
const Scene: FC<{ scene: AiDaily20260720Scene; index: number }> = ({ scene, index }) => {
  const frame = useCurrentFrame();
...
export const AiDaily20260720Video: FC<{ readonly scenes: readonly AiDaily20260720Scene[] }> = ({
  scenes,
}) => {
  const timelineScenes = scenes.map((scene, index) =>
    index === scenes.length - 1
      ? { ...scene, durationInFrames: scene.durationInFrames + AI_DAILY20260720_END_HOLD_IN_FRAMES }
      : scene,
  );
