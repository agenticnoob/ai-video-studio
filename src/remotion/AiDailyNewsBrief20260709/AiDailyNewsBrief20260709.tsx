...
import { aiDailyNewsBrief20260709Data } from "./data";
import {
  ...
  type AiDailyNewsBrief20260709Scene,
} from "./types";
...
export const getAiDailyNewsBrief20260709Duration = (data: AiDailyNewsBrief20260709Data): number =>
...
const SceneShell: FC<{
  ...
  readonly scene: AiDailyNewsBrief20260709Scene;
  ...
  <AbsoluteFill style={fullFrame}>
    ...
    >
      <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
        ...
        <div style={{ color: palette.muted, fontSize: 24, fontWeight: 850 }}>2026-07-09</div>
      </div>
      {children}
    </div>
...
const ThesisScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
    ...
    >
      ...
      <div style={{ display: "grid", gap: 22 }}>
        {[
          ["发布闸门", "受控预览 / 政府关注 / 分阶段公开", palette.yellow],
          ["算力主权", "自研芯片 / 50MW / 14GW", palette.cyan],
          ["电网设备", "变压器 / 开关设备 / 160 周", palette.green],
...
const ModelGateScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
...
const ModelMatrixScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
...
const AccessMapScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
  const nodes: WorkflowMapNode[] = [
    { id: "capital", label: "国家资本", tint: palette.yellow, x: 110, y: 100 },
    { id: "compute", label: "专用算力", tint: scene.accent, x: 345, y: 220 },
    { id: "language", label: "本地语言", tint: palette.cyan, x: 590, y: 100 },
    { id: "industry", label: "行业模型", tint: palette.rose, x: 835, y: 220 },
  ];
...
const InfraStackScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
    <div style={{ display: "grid", gap: 56, gridTemplateColumns: "1fr 1fr" }}>
      ...
      <div style={{ display: "grid", gap: 18 }}>
        {[
          ["1GW", "初始数据中心容量", palette.green],
          ["1.8GW", "可扩展容量上限", scene.accent],
          ["冷气候", "冷却条件更友好", palette.rose],
          ["能源合同", "电力与并网前置", palette.cyan],
...
const SecurityAuditScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell compactHeader scene={scene}>
    <BigTitle>{scene.headline}</BigTitle>
...
const SovereigntyScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
    <div style={{ display: "grid", gap: 62, gridTemplateColumns: "0.9fr 1.1fr" }}>
      ...
      <ContentCard3D tint={scene.accent}>
        <TimelineProgressBlock
          ...
          checkpointLabels={["GPU", "CUDA", "云算力", "并购"]}
          durationInFrames={scene.durationInFrames}
          mutedColor={palette.muted}
          note="监管开始从模型输出，延伸到芯片供应、软件生态和算力分配。"
...
const CapitalStackScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
  const metrics: MetricCardDatum[] = [
    { label: "Meta 目标", suffix: "GW", tint: scene.accent, value: 14 },
    { label: "Humain", suffix: "MW", tint: palette.rose, value: 50 },
    { label: "Alberta 初始", suffix: "GW", tint: palette.yellow, value: 1 },
  ];
...
const PowerGridScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
      ...
      <div style={{ display: "grid", gap: 30 }}>
        ...
        >
          <ContentCard3D tint={scene.accent}>
            ...
            >
              <BarChart
                ...
                height={370}
                padding={56}
                subtitle="视觉指数，用来表达瓶颈方向，不代表精确百分比"
                title="电网设备压力"
                width={790}
                yMax={100}
              />
          ...
          <div style={{ display: "grid", gap: 20 }}>
            {[
              ["设备端", "关键设备交付周期被数据中心需求拉长。", scene.accent],
              ["工程端", "并网、变电和冷却施工决定上线节奏。", palette.yellow],
              ["成本端", "电力容量会反向影响模型 API 稳定性。", palette.green],
            ].map(([label, copy, tint], index) => (
              <ContentCard3D index={index} key={label} tint={tint}>
                <Kicker
...
const RiskBoardScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
    <div
...
const PlaybookScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
  const nodes: WorkflowMapNode[] = [
...
const ClosingScene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => (
  <SceneShell scene={scene}>
...
const Scene: FC<{ readonly scene: AiDailyNewsBrief20260709Scene }> = ({ scene }) => {
...
export const AiDailyNewsBrief20260709Video: FC<{
  readonly data?: AiDailyNewsBrief20260709Data;
}> = ({ data = aiDailyNewsBrief20260709Data }) => (
  <AbsoluteFill style={{ background: palette.background }}>
    <StandaloneTimeline
      renderAudio={(scene) => (
        <StandaloneVoiceover
          audioFile={scene.audioFile}
          playbackRate={AI_DAILY_NEWS_BRIEF_20260709_VOICEOVER_PLAYBACK_RATE}
        />
      )}
...
export const aiDailyNewsBrief20260709Metadata = {
  durationInFrames: AI_DAILY_NEWS_BRIEF_20260709_DURATION_IN_FRAMES,
};
